...
def parse_info(file_name):
    f = open(file_name)
    ls = [line.split('\t') for line in f.readlines()]
    f.close()
    return ls
...
def print_info():
    ...
    for dic in info:
        print '{0}\t{1}\t{2}\t{3:7.2f}'.format(dic['order_date'].strftime('%Y-%m-%d'), dic['execution_date'].strftime('%Y-%m-%d'), dic['info_str'][:10].ljust(10), dic['value'])
        
def print_month_info():
    print '\nMonth\t' + '\t'.join(cat.rjust(7) for cat in categories)
    for month, dic in month_info.iteritems():
        print '{0}\t{1:7.2f}\t{2:7.2f}\t{3:7.2f}\t{4:7.2f}'.format(month, *[dic[cat] for cat in categories])
    print '*' * 40
    print 'Total\t{0:7.2f}\t{1:7.2f}\t{2:7.2f}\t{3:7.2f}'.format(*[sum(dic[cat] for dic in month_info.itervalues()) for cat in categories])
